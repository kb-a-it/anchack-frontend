...
async function save() {
  ...
  try {
    ...
    if (status === 401) {
      ...
      submitError.value =
        responseMessage || '본인이 작성한 리뷰만 수정할 수 있습니다.'
    } else if (status === 400) {
      submitError.value =
        responseMessage || '리뷰 입력 내용을 확인해주세요.'
    } else {
      submitError.value =
        responseMessage || '리뷰 수정에 실패했습니다. 잠시 후 다시 시도해주세요.'
    }
  } finally {
  ...
  >
    ...
    >
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          <label class="block text-sm font-semibold text-foreground mb-3">
            항목별 별점 <span class="text-red-500">*</span>
          </label>

          <div class="bg-muted/40 rounded-xl p-4 space-y-3">
            <div
              v-for="cat in REVIEW_CATEGORIES"
              :key="cat"
              class="flex items-center gap-4"
            >
              <span class="text-sm font-medium text-foreground w-16 flex-shrink-0">
                {{ cat }}
              </span>
              ...
              <StarInput
                ...
                :size="20"
                @update:model-value="(value) => (catRatings[cat] = value)"
              />
